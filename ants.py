...
class ThrowerAnt(Ant):
    ...
    #min and max ranges, to be overwritten by subclasses
    min_range = 0
    max_range = float('inf')
    ...
    def nearest_bee(self, beehive):
        ...
        while not(current_place is beehive):
            if len(current_place.bees) > 0 and (self.min_range <= i <= self.max_range):
...
class ShortThrower(ThrowerAnt):
    ...
    max_range = 3 #override max_range
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        # BEGIN Problem 6
        if self.digesting == 0:
            #Find a random bee in its place
